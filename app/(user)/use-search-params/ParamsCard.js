...
export default function ParamsCard() {
	...
	useEffect(() => {
	  ...
	  const isMale = useParams.gender === "male";

	  const confidentPer= useParams.probability*100
	//   if (isLoading){
	// 	return "loading"
	//   }
	return (
			...
			<div className="parent">
				<div className="card">
					...
					<div className="content">
						<span className="title">genderize</span>
						<span className="text">Name: {useParams.name}</span>
						<span className={`text ${isMale ? "red":"white"}`}>Gender: {useParams.gender}</span>
						<span className="text">Probability: {confidentPer}</span>
					</div>
